...
func update(player: Player, delta: float):
	...
	player.velocity.x = move_toward(player.velocity.x, 0, (player.stats.walk_speed/player.stats.accel_time) * delta)
	player.velocity.z = move_toward(player.velocity.z, 0, (player.stats.walk_speed/player.stats.accel_time) * delta)
